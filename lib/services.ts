...
export const wxMessages = async (limit = 50): Promise<WxMessage[]> => {
...
// 人设 / LLM 行为参数
export const getPersona = () => api.get<PersonaData>('/api/persona')
export const putPersona = (body: Partial<PersonaData>) =>
  api.put<{ updated: string[]; need_restart: boolean }>('/api/persona', body)
export const exportPersona = () => api.get<PersonaData>('/api/persona/export')
export const importPersona = (bundle: PersonaData) =>
  api.post<{ ok: boolean }>('/api/persona/import', bundle)