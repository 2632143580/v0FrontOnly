...
interface ConsoleState {
  ...
  dismissRestart: () => void
  savePersona: (changed: Partial<PersonaData>) => Promise<void>
  exportPersona: () => Promise<PersonaData | null>
  importPersona: (bundle: PersonaData) => Promise<void>
}
...
export function ConsoleProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    svc
      ...
      .catch(() => {
        if (alive) setPersonaLoaded(true)
      })
    return () => {
      alive = false
    }
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      es.addEventListener('tool_call', (e) => {
        try {
          const d = JSON.parse((e as MessageEvent).data)
          setToolCalls((prev) => [...prev, d].slice(-20))
        } catch {}
      })
      es.onerror = () => {
        es?.close()
  ...
  const savePersona = useCallback(
    async (changed: Partial<PersonaData>) => {
      setSavingPersona(true)
      try {
        await svc.putPersona(changed)
        const fresh = await svc.getPersona()
        setPersona(fresh)
        toast('info', '人设已保存')
      } catch (err) {
        const msg = err instanceof Error ? err.message : '保存失败'
        toast('error', `人设未保存 - ${msg}`)
      } finally {
  ...
  const exportPersona = useCallback(async (): Promise<PersonaData | null> => {
    try {
      const bundle = await svc.exportPersona()
      const blob = new Blob([JSON.stringify(bundle, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `persona-${new Date().toISOString().slice(0, 10)}.json`
      a.click()
  ...
  const importPersona = useCallback(
    async (bundle: PersonaData) => {
      try {
        await svc.importPersona(bundle)
        const fresh = await svc.getPersona()
        setPersona(fresh)
        toast('info', '人设已导入')
      } catch (err) {
